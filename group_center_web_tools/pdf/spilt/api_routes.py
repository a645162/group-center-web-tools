...
from .config import UPLOAD_DIR, OUTPUT_DIR, get_output_file_name
...
async def download_file(task_id: str, type: str):
    ...
    input_pdf = task_dict[task_id]
    file_name = os.path.basename(input_pdf)

    output_file_name = get_output_file_name(file_name, type)

    file_path = OUTPUT_DIR / output_file_name
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="File not found")

    return FileResponse(file_path, filename=output_file_name)
